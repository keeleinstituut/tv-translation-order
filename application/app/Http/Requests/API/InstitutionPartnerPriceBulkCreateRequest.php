...
class InstitutionPartnerPriceBulkCreateRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            collect($this->data)->each(function ($element, $index) use ($validator) {
                NestedFormRequestValidator::formRequest(new InstitutionPartnerPriceCreateRequest())
                    ->setData($element)
                    ->validate()
                    ->setMessagesToValidator($validator, "data.$index");
            });

            // Check within-batch uniqueness on (institution_partner_id, src, dst, skill) tuples
            $tuples = collect($this->data)->map(fn ($item) => implode('|', [
                $item['institution_partner_id'] ?? '',
            ...
            $duplicates->keys()->each(function ($index) use ($validator) {
                $msg = 'Duplicate institution partner, language pair and skill within the batch';
                $validator->errors()
                    ->add("data.$index.institution_partner_id", $msg)
